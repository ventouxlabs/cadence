...
- ~~The "challenge met" badge shows no date~~ **Resolved 2026-09-11** (D-232). `challenge.met_on` exists, `close_met` sets it to the `recorded_on` of the retest that reached the target — the day it happened, not the day it was typed in — and the badge names it: "Challenge met · earned 22 Mar". The column reaches an existing database too: `init_db` applies an additive `ALTER TABLE ... ADD COLUMN` on boot (D-283), because `Challenge` is ORM-mapped and a missing column broke `active_for` — and therefore every assessment — not merely the badge. A challenge already `met` before the column existed keeps a null `met_on` and still reads "Challenge met · earned" with no day, which is the honest answer for a day nobody recorded.
